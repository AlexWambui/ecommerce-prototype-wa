...
const imagePreview = ref<string | null>(props.brand.thumbnail_url);
...
const handleSubmit = () => {
    ...
    form.post(brandRoutes.update(props.brand.uuid).url, {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    ...
    <div class="form max-w-4xl mx-auto p-6">
        <FormHeader :backUrl="brandRoutes.index().url" title="Edit Brand" />

        <Form 
            :action="brandRoutes.update(props.brand.uuid).url" 
            method="post" 
                    ...
                    <div class="inputs-group">
                        <div class="flex items-center gap-2">
                            <input 
                                ...
                                :checked="props.brand.is_active"
                                value="1"
                                class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                            />
                            <Label for="is_active" class="cursor-pointer">Active</Label>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">Inactive brands won't be visible to customers</p>
                    </div>
                </div>

                <h3 class="text-lg font-semibold mb-4">Brand Image</h3>
                ...
                <div class="inputs-group">
                    <div class="flex flex-col justify-center">
                        <div class="relative w-48 h-48">
                            ...
                            <button
                                v-if="imagePreview && imagePreview !== props.brand.thumbnail_url"
                                type="button"
                                @click="removeLogo"
                        ...
                        <p class="text-xs text-gray-400 mt-2">
                            Current: {{ props.brand.image || 'No image uploaded' }}
                        </p>
